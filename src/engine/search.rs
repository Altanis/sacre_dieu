...
impl Searcher {
    ...
    pub fn search<const PV: bool>(&mut self, old_board: &Board, depth: usize, ply: usize, mut alpha: i32, beta: i32) -> i32 {
        ...
        if !PV && !in_check && static_eval >= beta {
            ...
            let nmp_score = -self.search::<false>(&nmp_board, ((depth as isize - 3) - (depth as isize / 3)).max(0) as usize, ply + 1, -beta, -alpha);
            if nmp_score >= beta {
                return nmp_score;
            }
        }
        ...
        for piece_move in moves.iter() {
            ...
            // Futility Pruning
            let is_mated = (SHALLOWEST_PROVEN_LOSS..DEEPEST_PROVEN_LOSS).contains(&best_score);
